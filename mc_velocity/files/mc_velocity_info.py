...
def get_minecraft_paper_versions():
    ...
    AvailableVersions = json_fetcher(_ApiBaseURL + "/" + _ApiEndVelocity)
    ...
    if (AvailableVersions["project_id"].lower() != "velocity") or (
        AvailableVersions["project_name"].lower() != "velocity"
    ):
...
def get_minecraft_paper_builds(Version: str):
    ...
    AvailableBuilds = json_fetcher(
        _ApiBaseURL + "/" + _ApiEndVelocity + "/" + _ApiEndVersions + "/" + Version
    )
    Entries = ["project_id", "project_name", "version", "builds"]

    json_check_return_entries(Entries=Entries, CheckDict=AvailableBuilds)

    if (AvailableBuilds["project_id"].lower() != "velocity") or (
        AvailableBuilds["project_name"].lower() != "velocity"
    ):
        raise ValueError("API not according to standard! Aborting...")
...
def get_minecraft_paper_download_info(Version: str, Build: str) -> MinecraftPaperInfo:
    ...
    if (PaperInfo["project_id"].lower() != "velocity") or (
        PaperInfo["project_name"].lower() != "velocity"
    ):
        raise ValueError("API not according to standard! Aborting...")
